...
//		 add_action( 'woocommerce_checkout_create_order_tax_item',                array( $this, 'create_tax_line_item'), 10, 3 );
		// Synch Changes
		add_action( 'woocommerce_order_status_changed', array( $this, 'sync_order_status_with_parent' ), 10, 4 );
...
//		add_action( 'before_delete_post', array( $this, 'delete_order_items' ) );
	...
	public function create_vendor_order( $args = array() ) {
		...
		$vendor_order->set_parent_order( $order_args['parent_order'] );
		$vendor_order->set_vendor_id( $order_args['vendor_id'] );
		$vendor_order->add_items( $order_args['items'] );
//		foreach ( $order_args['items'] as $item ) {
//			$this->add_item( $vendor_order, $item );
//		}
	...
	/**
	 * @param $vendor_order WCVendors_Vendor_Order
	 * @param $cart_item WC_Order_Item_Product
	 *
	 * @return mixed
	 * @throws Exception
	 */
	public function add_item( $vendor_order, $cart_item ) {
		$item_id = $vendor_order->add_product(
			$cart_item->get_product(),
			$cart_item->get_quantity(),
			array(
				'subtotal'     => $cart_item->get_subtotal(),
				'subtotal_tax' => $cart_item->get_subtotal_tax(),
				'total'        => $cart_item->get_total(),
				'total_tax'    => $cart_item->get_total_tax(),
				'tax'          => $cart_item->get_taxes(),
			)
		);

		if ( ! $item_id ) {
			// translators: placeholder is an internal error number
			throw new Exception( __( 'Unable to add item to vendor order.', 'wc-vendors' ) );
		}
	...
	*/
	 ...
	 */
	public function sync_order_status_with_parent( $order_id, $from, $to, $order ) {
		...
		$vendor_orders = wc_get_orders( $args );

		foreach ( $vendor_orders as $vendor_order ) {
			$vendor_order->update_status( $to, __( 'Status changes due to parent order.', 'wc-vendors' ) );
		}
	}
